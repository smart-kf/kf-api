...
func (r *QRCodeDomainRepository) DisableOld(ctx context.Context, cardId string, notId int64) error {
	tx := mysql.GetDBFromContext(ctx)
	if err := tx.Model(&dao.KFQRCodeDomain{}).Where("card_id = ? and id != ?", cardId, notId).Update(
		"status",
